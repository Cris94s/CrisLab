...
.then(data => {
    ...
    const detail = getGame("Sonic Frontiers");
    console.log(detail)
    
    let bgGame = document.querySelector('#bg-game');
    bgGame.style.background = `linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url("${detail.background_image}")`
    bgGame.style.backgroundSize = "cover"
    bgGame.style.backgroundPosition  = "center"
    bgGame.style.backgroundRepeat = "no-repeat"
